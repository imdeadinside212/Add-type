...
})();"""  # AI LOSY site (old per-page box-layer layout)

SCRIPT_CU_JS = r"""/**
 * STEP 1 — Paste in browser DevTools Console (F12 → Console tab).
 * Works on: ailosy.com viewer — NEW SITE LAYOUT (single shared box-layer)
 *
 * v10-ailosy — REWRITTEN FOR NEW DOM STRUCTURE
 *
 *  WHAT CHANGED ON THE SITE:
 *   • Old layout: one box-layer wrapper PER PAGE
 *     (".box-layer__box-wrapper" + ".losy-editor-viewer-layer-image__panel"),
 *     boxes were ".editor-box" with per-page-relative translate() coords.
 *   • New layout: ONE single ".box-layer" div for the WHOLE chapter. Every
 *     text box is an <article data-v-a9a68f4e class="box ..."> with:
 *       - data-item='{"id":1,"position":{"x":10,"y":61},
 *                      "size":{"width":339,"height":51},
 *                      "content":"<p>gasp</p>"}'
 *       - style="transform: translate(10px, 61px); width: 339px; height: 51px;"
 *     The transform / data-item position is a GLOBAL coordinate already —
 *     i.e. it is measured against the whole chapter's stacked images at
 *     their CSS *rendered* width (the <img> elements all render at a fixed
 *     display width, e.g. 320px, regardless of native PSD resolution).
 *
 *  WHAT STAYED THE SAME:
 *   • Page (image) native size still comes from each <img>'s title attribute:
 *       title="017.psd\n1440x19175\n300dpi"
 *   • renderScale is still derived per-page from
 *       img.getBoundingClientRect().height / nativeHeight
 *     so it's correct regardless of zoom/viewport/devicePixelRatio.
 *   • Output still carries BOTH per-page ratios (xRatio/yRatio) and
 *     global ratios (xRatioGlobal/yRatioGlobal) for Photoshop AUTO/SINGLE
 *     vs COMBINED placement modes.
 *   • Same duplicate-page guard + repeated-filename sanity check before
 *     download, since lazy-loaded pages can still double up in the DOM.
 *
 *  NOTE ON GLOBAL COORDS: because the new site already stores box positions
 *  in one shared global layer, we no longer need to add each box's cssY to
 *  a per-page offset to compute the global coordinate — the box's own
 *  translate() Y *is* already the global CSS Y. We still need the per-page
 *  cssY0/cssY1 boundaries (measured from each <img>'s DOM rect) purely to
 *  figure out WHICH page a box belongs to, and to convert global CSS Y into
 *  a page-relative ratio.
 */

(function extractTextBoxes() {

  // ── 1. Find the box layer ────────────────────────────────────────────────
  // New site only has ONE box-layer for the whole chapter (no per-page
  // wrapper to choose between), but we still guard against multiple/hidden
  // copies the same way the old script did.
  const allLayers = Array.from(document.querySelectorAll('.box-layer'));
  let container = allLayers.find(el => {
    const s = el.getAttribute('style') || '';
    return !s.includes('display: none') && !s.includes('display:none');
  }) || allLayers.find(el => el.querySelector('article.box')) || allLayers[0];

  if (!container) {
    console.error('❌ Could not find the .box-layer element.');
    return;
  }
  console.log('✅ box-layer found.');

  // ── 2. Build page table from each page <img>'s title attribute ──────────
  //
  //  The page images no longer live inside per-page "panel" wrapper divs —
  //  they're plain <img title="NNN.psd\nWxH\nDPI" ...> elements rendered in
  //  order. We grab every such img directly.

  const allImgs = Array.from(document.querySelectorAll('img[title]'))
    .filter(img => /\.(psd|png|jpg|jpeg|tiff?)\s*\n\s*\d+x\d+/i.test(img.getAttribute('title') || ''));

  if (allImgs.length === 0) {
    console.error('❌ No page <img title="NNN.psd\\nWxH\\n...dpi"> elements found.');
    return;
  }

  // ── 2a. De-duplicate pages by filename ───────────────────────────────────
  // Same rationale as the old script: lazily-mounted/re-rendered pages can
  // appear twice in the DOM with the same filename.
  const seenFilenames = new Map();
  const imgs = [];
  let duplicatesRemoved = 0;

  allImgs.forEach((img) => {
    const titleStr = img.getAttribute('title') || '';
    const fname = titleStr.split(/\r?\n/)[0];
    if (fname) {
      if (seenFilenames.has(fname)) {
        duplicatesRemoved++;
  ...
  if (duplicatesRemoved > 0) {
    console.warn(
      `⚠️  Removed ${duplicatesRemoved} duplicate page(s) with repeated filenames ` +
      `(found ${allImgs.length} <img> tags, kept ${imgs.length} unique pages). ` +
      `If this number looks unexpectedly large, reload the page (and make sure ` +
      `you've scrolled through the whole chapter so every page is mounted) then re-run.`
    );
  }
  ...
  imgs.forEach((img, idx) => {
    ...
    // ── Native dimensions from title: "017.psd\n1440x19175\n300dpi" ───────
    let nativeW = 1440, nativeH = 2000; // sane fallback
    const m = titleStr.match(/(\d+)x(\d+)/);
    if (m) { nativeW = parseInt(m[1], 10); nativeH = parseInt(m[2], 10); }

    // ── Rendered dimensions / position from the DOM ────────────────────────
    //
    // IMPORTANT (broken/unrendered page fallback): if this <img>'s CSS box
    // never got a real size, its bounding rect collapses to 0, which would
    // make this page's cssY0..cssY1 collapse to a single point, so every
    // box that belongs on it fails the `cssY0 <= cssY < cssY1` test and
    // gets pulled onto some other page instead — exactly the "boxes land
    // outside the image" symptom. We detect that and estimate this page's
    // height from the last successfully-measured page's scale (all pages
    // on a given site render at the same scale), chaining its top from the
    // previous page's bottom so the stacking order/height stays intact.
    //
    // NOTE: this does NOT require the image bytes to have actually
    // finished downloading (img.complete/naturalWidth). Pages whose CSS
    // gives them an explicit size (fixed width + aspect-ratio, or explicit
    // width/height) are correctly laid out as soon as CSS applies,
    // regardless of whether the pixel data has loaded -- requiring
    // naturalWidth here would wrongly mark perfectly-positioned-but-still-
    // downloading pages as broken.
    const rect = img.getBoundingClientRect();
    const loadedOk = rect.height > 0;
    ...
    if (loadedOk) {
      ...
      estimated = true;
      renderScale = lastGoodScale !== null ? lastGoodScale : (320 / nativeW);
      cssY0 = pages.length > 0 ? pages[pages.length - 1].cssY1 : 0;
      if (firstImgTop === null) firstImgTop = 0;
      cssY1 = cssY0 + nativeH * renderScale;
  ...
  // ── Assign cumulative nativeY0 for global *native-pixel* coords ─────────
  let cumNativeY = 0;
  pages.forEach(p => { p.nativeY0 = cumNativeY; cumNativeY += p.nativeH; });
  const totalNativeH = cumNativeY;
  const totalNativeW = pages[0].nativeW;
  ...
  // ── 3. Extract boxes ──────────────────────────────────────────────────────
  const boxEls = Array.from(container.querySelectorAll('article.box'));
  console.log(`\n🔍 ${boxEls.length} boxes in box-layer.`);
  ...
  // Strip inline formatting tags (<strong>, <em>, etc.) but keep their text,
  // and treat each <p> as one line — matches the old "p.paragraph" behavior.
  function htmlToLines(htmlStr) {
    const tmp = document.createElement('div');
    tmp.innerHTML = htmlStr || '';
    const ps = Array.from(tmp.querySelectorAll('p'));
    const source = ps.length > 0 ? ps : [tmp];
    return source
  ...
  boxEls.forEach((el, i) => {
    // ── Parse data-item JSON (authoritative for content; also a fallback
    //    for position/size if inline style parsing ever fails) ────────────
    let dataItem = null;
    try {
      dataItem = JSON.parse(el.getAttribute('data-item') || 'null');
    } catch (e) { /* ignore, fall back to DOM */ }

    // ── Text: each <p> inside data-item.content (or the live DOM) → one line ─
    let lines = dataItem && dataItem.content ? htmlToLines(dataItem.content) : [];
    if (lines.length === 0) {
      // Fallback: read straight from the rendered box body in the DOM.
      const bodyPs = Array.from(el.querySelectorAll('.tiptap p'));
      lines = bodyPs.map(p => p.innerText.trim()).filter(t => t.length > 0);
    }
    const text = lines.join('\n');
    if (!text) { skipped++; return; }

    // ── Mark (e.g. "#1" → "B1") ─────────────────────────────────────────
    // This site shows a plain number badge in a
    // <span class="box__body__mask-id">#1</span> inside the box (no "B"
    // letter here, unlike the other site's "#B93"). We prepend "B" so
    // layer naming stays consistent across both extraction scripts.
    // Falls back to dataItem.id if the badge span isn't found/rendered.
    const maskEl  = el.querySelector('.box__body__mask-id');
    const maskRaw = maskEl ? maskEl.textContent.trim() : '';
    const numMatch = maskRaw.match(/(\d+)/);
    const markNum = numMatch ? numMatch[1]
      : (dataItem && dataItem.id != null ? String(dataItem.id) : '');
    const mark = markNum ? 'B' + markNum : '';
    if (!mark) {
      console.warn(`  ⚠️  [${i}] No mark number found for box — layer will be unlabeled.`);
    }
    // Ready-to-use Photoshop layer name: "B1_first 40 chars of text"
    const layerNameSafe = text.replace(/\n/g, ' ').slice(0, 40).trim();
    const layerName = mark ? `${mark}_${layerNameSafe}` : layerNameSafe;

    // ── Position/size from inline style transform (live DOM, authoritative) ─
    const styleStr = el.getAttribute('style') || '';
    const tMatch = styleStr.match(/translate\(([0-9.]+)px,\s*([0-9.]+)px\)/);

    let cssX, cssY, cssW, cssH;
    if (tMatch) {
      cssX = parseFloat(tMatch[1]);
      cssY = parseFloat(tMatch[2]); // global CSS Y — whole chapter is one stacked layer
      const wM = styleStr.match(/width:\s*([0-9.]+)px/);
      const hM = styleStr.match(/height:\s*([0-9.]+)px/);
      cssW = wM ? parseFloat(wM[1]) : (dataItem ? dataItem.size.width : 0);
      cssH = hM ? parseFloat(hM[1]) : (dataItem ? dataItem.size.height : 0);
    } else if (dataItem) {
      // Fallback to the data-item attribute if inline style is ever missing.
      cssX = dataItem.position.x;
      cssY = dataItem.position.y;
      cssW = dataItem.size.width;
      cssH = dataItem.size.height;
    } else {
      skipped++; return;
    }

    // ── Page assignment by global cssY ──────────────────────────────────
    let pi = pages.findIndex(p => p.cssY0 <= cssY && cssY < p.cssY1);
    if (pi === -1) {
      pi = pages.reduce((best, p, j) =>
        Math.abs(cssY - (p.cssY0 + p.cssH / 2)) <
    ...
    // ── Font size ─────────────────────────────────────────────────────────
    const fsAttr   = parseFloat(el.getAttribute('fontsize'));
    const fsCss     = !isNaN(fsAttr) ? fsAttr
      : (() => {
          const wrapper = el.querySelector('.tiptap__content');
          return wrapper ? parseFloat(getComputedStyle(wrapper).fontSize) || 10 : 10;
        })();
    const fsNative = fsCss / rs;

    // ── Color and alignment ─────────────────────────────────────────────────
    // Background color sits right on the box's own inline style
    // ("background-color: rgb(252, 220, 0)") — that's the highlight/box
    // color. By default the TEXT color just follows the box color (same as
    // before), EXCEPT for the yellow #FCDC00 boxes, whose text should
    // always render black for legibility.
    const bgMatch = styleStr.match(/background-color:\s*([^;]+);/);
    const bgColor = bgMatch ? bgMatch[1].trim() : (dataItem ? dataItem.color : 'rgb(0,0,0)');

    // Normalize bgColor (which may be "rgb(252, 220, 0)" or a hex string
    // from data-item.color like "#FCDC00") to a hex string for comparison.
    function toHex(c) {
      if (!c) return '';
    ...
    const YELLOW_BOX_HEX = '#FCDC00';
    const color = toHex(bgColor) === YELLOW_BOX_HEX ? '#000000' : bgColor;
    const textAlign = el.getAttribute('textalign')
      || (() => {
            const wrapper = el.querySelector('.tiptap__content');
            return wrapper ? (getComputedStyle(wrapper).textAlign || 'center') : 'center';
          })();
    ...
    pageResults[pi].textBoxes.push({
      id: (dataItem && dataItem.id != null) ? dataItem.id : i,
      text,
      mark,               // e.g. "B1" (empty string if not found)
      layerName,          // e.g. "B1_rustle" — ready to use as PS layer name
      ...
      fontSizeNativePx:    +fsNative.toFixed(2),
      fontSizeRatio:       +(fsNative / page.nativeH).toFixed(6),
      fontSizeRatioGlobal: +(fsNative / totalNativeH).toFixed(6),
  ...
  const unmarkedCount = pageResults.reduce(
    (n, p) => n + p.textBoxes.filter(b => !b.mark).length, 0);
  if (unmarkedCount > 0) {
    console.warn(`\n⚠️  ${unmarkedCount} box(es) have no mark — check .box__body__mask-id exists for them.`);
  }
  ...
  if (filenameList.length % 2 === 0) {
    ...
    if (firstHalf === secondHalf) {
      repeatWarning =
        `⚠️⚠️⚠️  The ${filenameList.length} pages look like the SAME ${half} filenames, ` +
        `repeated twice in a row. This almost always means the page content was captured ` +
        `twice (e.g. duplicate <img> tags, or the script ran while the viewer was still ` +
        `re-rendering). The downloaded file may have DOUBLE the real canvas height, which ` +
        `will throw off every position calculation in Photoshop.\n` +
        `   → Reload the page, wait for it to fully finish loading, then re-run this script.`;
    }
  ...
  const output = {
    extractedAt:      new Date().toISOString(),
    pageURL:          window.location.href,
    scriptVersion:    'v12-ailosy-newlayout-broken-img-fallback',
    coordSystem:      'ratios — per-page (xRatio/yRatio) AND global (xRatioGlobal/yRatioGlobal)',
    totalPages:       pages.length,
    totalTextBoxes:   assigned,
    totalNativeWidth: totalNativeW,
  ...
  console.log(`\n🎉 Downloaded textboxes.json — ${pages.length} page(s), ${assigned} box(es).`);
  if (duplicatesRemoved > 0) {
    console.log(`   (${duplicatesRemoved} duplicate page(s) were auto-removed before export.)`);
  }
  console.log('   If any ⚠️ warnings appeared above, check that all page images are');
  console.log('   fully loaded and visible (scroll through the whole chapter once) before');
  console.log('   running this script.');

})();"""  # LOSY CU site (new single shared box-layer layout)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if page.query_selector(".box-layer__box-wrapper"):
                ...
                site_label = "AI LOSY (old per-page layout)"
            elif page.query_selector("article.box") or page.query_selector(".box-layer"):
                script_text = SCRIPT_CU_JS
                site_label = "LOSY CU (new shared-layer layout)"
            else:
                raise RuntimeError(
                    "Could not detect the site layout from this file (no known "
                    "selectors found). Make sure the page fully loaded/rendered "
                    "before it was saved."
                )

            log("Detected site: " + site_label)

            # ── AI LOSY (old per-page layout) specific safeguard ──────────
            #
            ...
            if (not is_url) and script_text is SCRIPT_MOI_JS:
                try:
